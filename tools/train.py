...
from models.MSFR import MSFR_base, MSFR_pipeline_
...
def train(cfg=None):
    ...
    if cfg.DATASET.name in ["aebad_S", "mvtec"]:
    ...
    for idx, individual_dataloader in enumerate(train_dataloaders):
        ...
        if cfg.TRAIN.method in ['MSFR']:
            ...
            MSFR_base = MSFR_base(
                   cfg=cfg,
                                scale_factors=cfg.TRAIN.MSFR.scale_factors,
                                FPN_output_dim=cfg.TRAIN.MSFR.FPN_output_dim)
                # define the model

            
            if cfg.TRAIN.MSFR.load_pretrain_model:
                checkpoint = torch.load(cfg.TRAIN.MSFR.model_chkpt)
                checkpoint = scratch_MAE_decoder(checkpoint)
                LOGGER.info("train the decoder FPN of MSFR from scratch!")
                msg = MSFR_base.load_state_dict(checkpoint['model'], strict=False)
        ...
        if cfg.TRAIN.method in ['MSFR']:
            ...
            # param_groups = optim_factory.add_weight_decay(MSFR_base, cfg.TRAIN_SETUPS.weight_decay)
            param_groups = add_weight_decay(MSFR_base, cfg.TRAIN_SETUPS.weight_decay)
        ...
        if cfg.TRAIN.method == 'MSFR':
            MSFR_instance = MSFR_pipeline_(cur_model=cur_model,
                                         MSFR_model=MSFR_base,
                                         optimizer=optimizer,
                                         device=cur_device,
                                         cfg=cfg)
            # MSFR_instance.fit(individual_dataloader)
        else:
            raise NotImplementedError("train method {} does not include in target methods".format(cfg.TRAIN.method))
        ...
        for each_class in measured_list:
            ...
            """
            if cfg.TRAIN.method == 'MSFR':
                # MSFR_instance.load_model()
                auc_sample, auroc_pixel, pro_auc = MSFR_instance.evaluation(
