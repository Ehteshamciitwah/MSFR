from .utils import ForwardHook, cal_anomaly_map, each_patch_loss_function, MSFR_adjust_learning_rate
...
class MSFR_pipeline_:
    ...
    def __init__(self,
        ...
        for extract_layer in cfg.TRAIN.MSFR.layers_to_extract_from:
        ...
        self.MSFR_model = MSFR_model.to(device)

        self.optimizer = optimizer
        self.device = device
        self.cfg = cfg
        self.save_loss = 0.5
        self.save_fig_path = os.path.join(cfg.OUTPUT_DIR, "checkpoints/best.pth")
    ...
    def fit(self, individual_dataloader):
        ...
        for epoch in range(self.cfg.TRAIN_SETUPS.epochs):
            self.cur_model.eval()
            self.MSFR_model.train()
            current_lr = MSFR_adjust_learning_rate(self.optimizer, epoch, self.cfg)
            if (epoch + 1) % 50 == 0:
            ...
            for image in individual_dataloader:
                ...
                multi_scale_features = [self.teacher_outputs_dict[key]
                                        for key in self.cfg.TRAIN.MSFR.layers_to_extract_from]
                reverse_features = self.MSFR_model(image,
                                                  mask_ratio=self.cfg.TRAIN.MSFR.finetune_mask_ratio)  # bn(inputs))
            ...
            if self.save_loss > np.mean(loss_list):
                self.save_loss = np.mean(loss_list)
                self.save_model()
    ...
    def evaluation(self, test_dataloader):
        ...
        with torch.no_grad():
            for image in test_dataloader:
                ...
                """
                reverse_features = self.MSFR_model(image,
                                                  mask_ratio=self.cfg.TRAIN.MSFR.test_mask_ratio)
                multi_scale_reverse_features = [reverse_features[key]
                                                for key in self.cfg.TRAIN.MSFR.layers_to_extract_from]
    ...
    def save_model(self):
        try:
            torch.save(self.MSFR_model.state_dict(), self.save_fig_path)
            print("Model saved successfully.")
        except Exception as e:
            print(f"Error occurred while saving the model: {e}. Continuing...")

    def load_model(self):
        try:
            self.MSFR_model.load_state_dict(torch.load(self.save_fig_path))
            print("Model loaded successfully.")
        except Exception as e:
            print(f"Error occurred while loading the model: {e}. Continuing...")
